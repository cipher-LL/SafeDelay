...
export class CrowdFund {
    ...
    connect(artifact, config) {
        this.contract = new Contract(artifact, [config.creatorPublicKeyHash, config.fundingGoal, config.deadline], { provider: this.provider });
        this.config = config;
    }
    /**
     ...
     */
    async contribute(backerKey, amount) {
        this.requireConnected();
